...
from .asciiutils import AsciiLenGetIter
...
class Header(object):
    ...
    def __init__(self, filename=None, comment_char=None):
        ...
        # come as vectors
        self.SexVectorColls = ('MAG_APER', 'MAGERR_APER', 'FLUX_RADIUS', 'FLUX_APER', 'FLUXERR_APER', 'VECTOR_SOMFIT',
                               'VECTOR_ASSOC', 'FLUX_GROWTH', 'VIGNET', 'VIGNET_SHIFT')
    ...
    def _load_header(self, filename, comment_char):
        ...
        sextractor_header = re.compile(r'^#\s*(\d+)\s+([+*-/()\w]+)([^\[]*)(\[\w+\])?(.*)\n')
